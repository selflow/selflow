...
const getDepth = (stepId: string, stepMap: Record<string, WorkflowStep>): number => {
  ...
  return Math.max(...stepMap[stepId].needs.map(dependency => getDepth(dependency, stepMap) + 1))
}

export const mapWorkflowStepToReactFlowNodeAndEdges = (steps: WorkflowStep[]): [Node<WorkflowStepProps>[], Edge[]] => {
  ...
  for (let step of steps) {
    ...
    edges.push(...step.needs.map(dependency => ({
