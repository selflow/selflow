...
export const EditStepForm = ({initialStep}: EditStepFormProps) => {
  ...
  return <>
    ...
    <form className={"mt-5"} onSubmit={onSubmit}>
      ...
      <Input type="text" label={"Step Id"} {...register('id', {required: true})}/>

      <Controller
        name={"needs"}
        control={control}
        defaultValue={[]}
        render={({field}) => <MultiSelect items={
          steps.map(step => step.id)
        } placeholder={"Pick the step dependencies"} label={"Dependencies"} onChange={field.onChange}
                                          initialSelectedItems={field.value}/>}
      />

      <h2 className={"text-xl mt-5"}>Container Configuration</h2>

      <Input type="text" label={"Docker Image"} {...register('with.image', {required: true})}/>

      <Controller
        name={'with.commands'}
        control={control}
        defaultValue={''}
        render={({field}) => <CodeEditor lang={'sh'} label={"Commands"} value={field.value} onChange={field.onChange}/>}
      />
